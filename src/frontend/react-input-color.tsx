...
export default createReactClass({
    ...
    componentWillMount() {
        this.changeRequestsed = false;
        return this.lastRequestedValue = this.props.valueLink.value;
    },

    componentDidMount() {
        const ctrl = this.refs.native_ctrl;
        ...
        ctrl.type = "color";
        ctrl.value = this.lastRequestedValue;
        return ctrl.onchange = this.debouncedRequestChange;
    },

    debouncedRequestChange() {
        // see module-level comment on approach to debouncing
        this.requestedValue = this.refs.native_ctrl.value;
        if (this.changeRequestsed) { return; }
        window.requestAnimationFrame(this.doUpdate);
        return this.changeRequestsed = true;
    },
    ...
    componentDidUpdate() {
        ...
        if (this.props.valueLink.value === this.lastRequestedValue) { return; }

        const passed_in_value = this.props.valueLink.value;

        // set the color picker to the color passed in from "outside"
        this.refs.native_ctrl.value = passed_in_value;

        // drop any pending updates
        return this.lastRequestedValue = passed_in_value;
    },
    ...
    doUpdate() {
        ...
        this.lastRequestedValue = this.requestedValue;
        ...
        [this.requestedValue, this.changeRequestsed] = Array.from([null, false]);

        // tell the outside world we've changed
        return this.props.valueLink.requestChange(this.lastRequestedValue);
    }
});
