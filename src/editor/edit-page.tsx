...
const {
  remapSymbolsToExistingComponents,
  prettyPrintDocDiff,
...
import {
  MutlistateHoleBlock,
  MutlistateAltsBlock,
} from '../blocks/non-component-multistate-block'
const {
  OvalBlockType,
  TriangleBlockType,
  LineBlockType,
...
defaultExport.Editor = Editor = createReactClass({
  ...
  render() {
    if (_l.size(this.librariesWithErrors) > 0) {
      const lib_in_dev_mode = _l.find(this.librariesWithErrors, {
        inDevMode: true,
      })

      const cli_running = !_l.some(
        lib_in_dev_mode != null
          ? lib_in_dev_mode.loadErrors(window)
          : undefined,
        err => err.__pdStatus === 'net-err'
      )
      return React.createElement(
        ErrorPage,
        {
          message:
            lib_in_dev_mode && !cli_running
          ...
          detail:
            lib_in_dev_mode && !cli_running
              ? React.createElement(
                  'a',
                  { href: 'https://documentation.pagedraw.io/cli/' },
                  'Click here to install the pagedraw CLI'
                )
              : undefined,
        },
        React.createElement(
          ...
          this.librariesWithErrors.map(lib => {
            ...
            return React.createElement(
              ...
              React.createElement(
                ...
                lib.inDevMode
                  ? React.createElement('div', null, 'In dev mode')
                  : undefined
              ),
              React.createElement(
                'div',
              ...
              error.__pdStatus === 'different-state-upon-load'
                ? React.createElement(
                    ...
                    {
                      onClick: () => {
                        if (this.docjsonThatWasPreventedFromLoading == null) {
                          return
                        }

                        return makeLibAtVersion(
                          window,
                          lib.library_id,
                          lib.version_id
                        ).then(new_lib => {
                          let found
                          const newCodeSpecRefs = new_lib.cachedExternalCodeSpecs.map(
                            ({ ref }) => ref
                          )

                          if (
                            (found = _l.find(
                              lib.cachedExternalCodeSpecs,
                              ({ ref }) =>
                                !Array.from(newCodeSpecRefs).includes(ref)
                            )) != null
                          ) {
                            // FIXME: Should alert the user instead of throwing
                            throw new Error(
                              'Reinstall failed. Would delete external code specs.'
                            )
                          }
                          const doc = Doc.deserialize(
                            this.docjsonThatWasPreventedFromLoading
                          )
          ...
          React.createElement(
            PdButtonOne,
            {
              onClick: () => {
                ...
                this.librariesWithErrors = []
                this.finishLoadingDoc(
                  Doc.deserialize(this.docjsonThatWasPreventedFromLoading),
                  this.docjsonThatWasPreventedFromLoading
                )
                return (this.docjsonThatWasPreventedFromLoading = null)
              },
            ...
            'Proceed without loading libraries'
          ),
    ...
    const shadowDom = ({ contents, wrapper }) => {
      if (config.shadowDomTheEditor) {
        return React.createElement(
          ShadowDOM,
          { includeCssUrls: config.editor_css_urls },
          wrapper(contents)
        )
      } else {
        return contents
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          'title',
          null,
          this.props.windowTitle != null
            ? this.props.windowTitle
            : `${this.doc.url} — Pagedraw`
        )
      ),
      font_loading_head_tags_for_doc(this.doc),
      shadowDom({
        wrapper: content => content,
        contents: React.createElement(ShouldSubtreeRender, {
          shouldUpdate:
            this.editorCache.render_params.dontUpdateSidebars !== true,
          subtree: () =>
            DraggableInElectron(
              editorMode.topbar(
                this,
                this.props.defaultTopbar != null
                  ? this.props.defaultTopbar
                  : config.nonComponentMultistates
                  ? 'with-mk-multi'
                  : 'default'
              )
            ),
          ...
          ? shadowDom({
              ...
              contents: React.createElement(
                ...
                React.createElement(
                  ...
                  React.createElement(ShouldSubtreeRender, {
                    shouldUpdate:
                      this.editorCache.render_params.dontUpdateSidebars !==
                      true,
                    subtree: () => {
        ...
        config.libraryPreviewSidebar
          ? [
              React.createElement(ShouldSubtreeRender, {
                ...
                key: 'sb',
                subtree: () => {
                  return React.createElement(LibraryPreviewSidebar, {
                    doc: this.doc,
                    setEditorMode: this.setEditorMode,
                    editorMode: editorMode,
                    onChange: this.handleDocChanged,
                  })
                },
              }),
              React.createElement('div', {
  ...
  topbarBlockAdder() {
    const trigger = React.createElement(
      'div',
      null,
      React.createElement(TopbarButton, {
        text: 'Add',
        image: 'https://ucarecdn.com/10ab7bf3-7f34-4d0f-a1b4-3187747c3862/',
      })
    )
    ...
    const popover = closePopover => {
      ...
      const item_for_type = blockType => {
        ...
        return React.createElement(MenuItem, {
          ...
          onClick: handler,
          label: keyCommand,
          key: blockType.getUniqueKey(),
        })
      }

      var external_code_entries = (node, key) => {
        if (node.ref != null) {
          return item_for_type(
            new ExternalBlockType(
              _l.find(this.doc.getExternalCodeSpecs(), { uniqueKey: node.ref })
            )
          )
        } else {
          return React.createElement(
            MenuItem,
            { text: node.name, key: `folder-${key}` },
            node.children.map(external_code_entries)
          )
        }
      }
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            MenuItem,
            { text: 'Document Component' },
            (block_types = user_defined_block_types_list(this.doc)).length > 0
              ? block_types.map(item_for_type)
              : React.createElement(MenuItem, {
                  text: 'Draw an artboard to define a component',
                  disabled: true,
                })
          ),
          config.realExternalCode
            ? React.createElement(
                ...
                React.createElement(MenuItem, {
                  ...
                  onClick: () => {
                    this.setEditorMode(new LibStoreInteraction())
                    closePopover()
                    return this.handleDocChanged({ fast: true })
                  },
                }),
                (children = this.doc.getExternalCodeSpecTree().children)
                  .length > 0
                  ? children.map(external_code_entries)
                  : React.createElement(MenuItem, {
                      text: 'No libraries added to this document yet',
                      disabled: true,
                    })
              )
            : undefined
    ...
    return React.createElement(Popover, {
      trigger: trigger,
      popover: popover,
      popover_position_for_trigger_rect(trigger_rect) {
        return {
  ...
  showUpdatingFromFigmaModal() {
    ...
    if ((access_token = window.pd_params.figma_access_token)) {
      window.history.replaceState(
        null,
        null,
        `/pages/${window.pd_params.page_id}`
      )
      ...
      config.realExternalCode
        ? React.createElement(
            ...
            React.createElement(
              ...
              'Add Libraries to Doc'
            ),
            React.createElement(
              PdSidebarButton,
              {
                onClick: () => libManagerModal(this.doc, this.handleDocChanged),
              },
              'Create Libraries'
            )
          )
      ...
      config.handleRawDocJson
        ? React.createElement(
                  ...
                  return modal.show(closeHandler => [
                    ...
                    React.createElement(
                      ...
                      React.createElement(
                        'p',
                        null,
                        "Hey, you found a Pagedraw Developer Internal Feature!  That's pretty cool, don't tell your friends."
                      ),
                      React.createElement(CodeShower, {
                        content: JSON.stringify(this.doc.serialize()),
                      })
                    ),
                    React.createElement(
                      Modal.Footer,
                      ...
                      React.createElement(
                        PdButtonOne,
                        { type: 'primary', onClick: closeHandler },
                        'Close'
                      )
                    ),
                  ])
            ...
            React.createElement(FormControl, {
              tag: 'textarea',
              style: { width: '100%' },
              valueLink: propLink(this, 'raw_doc_json_input', () =>
...
`
      ...
      (() => {
        if (config.crashButton) {
          ...
          return React.createElement(
            ...
            React.createElement(
              PdSidebarButton,
              { onClick: () => log_assert(() => false) },
              'Log assert false'
            ),
            React.createElement(
              PdSidebarButton,
              { onClick: () => log_assert(() => true) },
              'Log assert true'
            ),
            React.createElement(
              PdSidebarButton,
              { onClick: () => log_assert(() => true[0]['undefined']()) },
              'Log assert throws'
            ),
            React.createElement('hr', null)
          )
      ...
      !_l.isEmpty(this.doc.figma_url)
        ? React.createElement(
            ...
            window.pd_params.figma_access_token
              ? React.createElement(
                  PdSidebarButton,
                  { onClick: this.showUpdatingFromFigmaModal },
                  'Update from Figma'
                )
              : React.createElement(
                  'a',
                  {
                    href: `/oauth/figma_redirect?page_id=${window.pd_params.page_id}`,
                  },
                  React.createElement(
                    PdSidebarButton,
                    null,
                  ...
                  return modal.show(closeHandler => {
                    return [
                      ...
                      React.createElement(
                        Modal.Body,
                        null,
                        React.createElement(SketchImporter, {
                          onImport: doc_json => {
                            this.updateJsonFromSketch(doc_json)
                            return closeHandler()
                          },
                        })
                      ),
      ...
      config.configEditorButton
        ? React.createElement(
            ...
            React.createElement(
              PdSidebarButton,
              { onClick: window.__openConfigEditor },
              'Config'
            ),
            React.createElement('hr', null)
          )
      ...
      config.normalizeForceAllButton
        ? React.createElement(
            ...
            React.createElement(
              ...
              {
                onClick: () => {
                  this.doc.inReadonlyMode(() => this.normalizeForceAll())
                  return this.handleDocChanged({ fast: true })
                },
              },
              'Force Normalize All'
      ...
      config.diffSinceCommitShower && this.docRef != null
        ? React.createElement(
            ...
            React.createElement(
              ...
              {
                onClick: () => {
                  const diff = prettyPrintDocDiff(
                    this.cacheOfLastCommit.doc,
                    this.doc
                  )
                  return server.compileDocjson(
                    this.doc.serialize(),
                    compiled_head => {
                      return server.compileDocjson(
                        this.cacheOfLastCommit.doc.serialize(),
                        function(compiled_master) {
                          const zipped = zip_dicts(
                            [compiled_master, compiled_head].map(results =>
                              _l.keyBy(results, 'filePath')
                            )
                          )
                          const diff_results = _l.compact(
                            _l.map(zipped, function(...args) {
                              const [old_result, new_result] = Array.from(
                                  args[0]
                                ),
                                filePath = args[1]
                              if (old_result == null && new_result != null) {
                                ...
                                return [
                                  filePath,
                                  [{ color: 'red', line: old_result.contents }],
                                ]
                              } else if (
                                old_result == null &&
                                new_result == null
                                ...
                                old_result.contents !== new_result.contents
                              ) {
                                const diffedLines = _l.flatten(
                                  jsdiff
                                    .diffLines(
                                      old_result.contents,
                                      new_result.contents
                            ...
                            return [
                              ...
                              React.createElement(
                                ...
                                React.createElement(
                                  ...
                                  {
                                    defaultActiveKey: 0,
                                    id: 'commit-diffs-tabs',
                                  },
                                  diff_results.map((...args) => {
                                    const [filePath, diffedLines] = Array.from(
                                        args[0]
                                      ),
                                      key = args[1]
                                    return React.createElement(
      ...
      this.docRef != null
        ? React.createElement(HistoryView, {
            ...
            showDocjsonDiff: commits_docjson => {
              this.setEditorMode(
                new DiffViewInteraction(commits_docjson, this.doc.serialize())
              )
              return this.handleDocChanged({ fast: true, mutated_blocks: [] })
            },
          })
  ...
  hasUncommitedChanges() {
    ...
    if (
      lastCommitRef.uniqueKey !==
        (this.cacheOfLastCommit != null
          ? this.cacheOfLastCommit.uniqueKey
          : undefined) &&
      ...
      server.getCommit(this.docRef, lastCommitRef).then(serializedCommit => {
        ...
        if (this.cacheOfLastCommitRequestHash !== lastCommitRef.uniqueKey) {
          return
        }

        this.cacheOfLastCommitRequestHash = undefined
        this.cacheOfLastCommit = {
          doc: Doc.deserialize(serializedCommit),
          uniqueKey: lastCommitRef.uniqueKey,
        }
    ...
    if (
      (this.cacheOfLastCommit != null
        ? this.cacheOfLastCommit.uniqueKey
        : undefined) !== lastCommitRef.uniqueKey
    ) {
  ...
  assertSynchronousDirty() {
    ...
    const caller_stack_trace_holding_error = new Error(
      "handleDocChanged() wansn't called synchronously"
    )
    ...
    return if_changed({
      ...
      if (unchanged) {
        return console.error(caller_stack_trace_holding_error)
      }
    })
  },
  ...
  handleDocChanged(render_params) {
    ...
    if (this.props.readonly === true && !render_params.fast) {
      return this.swapDoc(this.lastDocFromServer)
    }
    ...
    log_assert(
      () =>
        !this.isRenderingFlagForAsserts ||
        (render_params.mutated_blocks != null &&
          _l.isEmpty(render_params.mutated_blocks))
    )
    ...
    return (callback_because_dirty_is_async => {
      if (
        config.onlyNormalizeAtTheEndOfInteractions &&
        this.editorCache.render_params.fast
      ) {
  ...
  enqueueForceUpdate(element, callback, dirtyAllCallback) {
    if (dirtyAllCallback != null) {
      this.dirtyAllCallbacks.push(dirtyAllCallback)
    }
    ...
    return element.forceUpdate(() => {
      ...
      if (this.enqueuedForceUpdates === 0) {
        for (callback of Array.from(this.dirtyAllCallbacks)) {
          callback()
        }
  ...
  dirtyAll(whenAllFinished) {
    ...
    return this.enqueueForceUpdate(
      { forceUpdate: callback => this.dirty(callback) },
      function() {},
      whenAllFinished
    )
  },
  ...
  getBlockMinGeometry(block) {
    return this._getBlockMinGeometry(
      block,
      this.getInstanceEditorCompileOptions(),
      ReactDOM.findDOMNode(this.refs.off_screen_div)
    )
  },
  _getBlockMinGeometry(block, instanceEditorCompilerOptions, offscreen_node) {
    if (!(block instanceof BaseInstanceBlock)) {
      ...
      let pdom = this.clean_pdom_for_geometry_computation(
        evalInstanceBlock(block, instanceEditorCompilerOptions)
      )
      pdom.width = 'min-content'

      let cache_entry = this.other_peoples_computed_instance_min_widths[
        block.uniqueKey
      ]
      ...
      const computedWidth = source.componentSpec.flexWidth
        ? Math.max(block.width, minWidth)
        : minWidth
      pdom = clonePdom(pdom)
      pdom.width = computedWidth
      cache_entry = this.other_peoples_computed_instance_min_heights[
        block.uniqueKey
      ]
  ...
  _fastGetBlockMinGeometry(
    block,
    instanceEditorCompilerOptions,
    offscreen_node
  ) {
    if (!(block instanceof BaseInstanceBlock)) {
      return { minWidth: 0, minHeight: 0 }
      ...
      const pdom = this.clean_pdom_for_geometry_computation(
        evalInstanceBlock(block, instanceEditorCompilerOptions)
      )
      pdom.width = 'min-content'
      ...
      const dom_element = () =>
        mounted != null
          ? mounted
          : (mounted = mountReactElement(pdomToReact(pdom), offscreen_node))
      ...
      if (mounted != null) {
        ReactDOM.unmountComponentAtNode(offscreen_node)
      }

      assert(() =>
        _l.isEqual(
          this._getBlockMinGeometry(
            block,
            instanceEditorCompilerOptions,
            offscreen_node
          ),
      ...
      return (() => {
        ...
        for (let block of Array.from(this.doc.blocks)) {
          ...
          if (block instanceof TextBlock) {
            pdom = this.clean_pdom_for_geometry_computation(
              block.pdomForGeometryGetter(compile_opts)
            )
            const cache = {
              pdom,
              height: block.height,
              width: block.computedSubpixelContentWidth,
            }
            result.push(
              (this.editorCache.blockComputedGeometryCache[
                block.uniqueKey
            ...
            try {
              var source
              if ((source = block.getSourceComponent()) == null) {
                continue
              }
              const [resizable_width, resizable_height] = Array.from([
                source.componentSpec.flexWidth,
                source.componentSpec.flexHeight,
              ])

              pdom = this.clean_pdom_for_geometry_computation(
                evalInstanceBlock(block, compile_opts)
              )
              pdom.width = 'min-content'
              ...
              this.other_peoples_computed_instance_min_widths[
                ...
                width: resizable_width
                  ? Math.min(computed.width, block.width)
                  : block.width,
              }
              ...
              result.push(
                (this.other_peoples_computed_instance_min_heights[
                  ...
                  height: resizable_height
                    ? Math.min(computed.height, block.height)
                    : block.height,
                })
              )
            } catch (error) {}
  ...
  normalize() {
    ...
    if (this.editorCache.render_params.mutated_blocks != null) {
      changed_blocks_by_uniqueKey = _l.keys(
        this.editorCache.render_params.mutated_blocks
      )

      // If no one told us we behave like grown ups and calculate it ourselves
    } else {
      const [old_docjson, next_docjson] = Array.from([
        this.last_normalized_doc_json,
        this.doc.serialize(),
      ])
    ...
    const instanceBlocksForComponent = _l.groupBy(
      this.doc.blocks.filter(b => b instanceof InstanceBlock),
      'sourceRef'
    )
    ...
    const newOwningComponentForBlock = _l.fromPairs(
      _l.flatten(
        Array.from(this.doc.getComponentBlockTrees()).map(blockTree =>
          (() => {
            const result1 = []
            for (let block of Array.from(blocks_from_block_tree(blockTree))) {
              result1.push([
                block.uniqueKey,
                blockTree.block.componentSpec.componentRef,
              ])
            }
            return result1
          })()
    ...
    const blocks_to_normalize = _l.compact(
      find_connected(changed_blocks_by_uniqueKey, block_uniqueKey => {
        return _l.flatten(
          (() => {
            const result2 = []
            for (let owningComponentForBlock of [
              this.oldOwningComponentForBlock,
              newOwningComponentForBlock,
            ]) {
              ...
              if (component === undefined) {
                ...
                result2.push(
                  _l.map(
                    instanceBlocksForComponent[component] != null
                      ? instanceBlocksForComponent[component]
                      : [],
                    'uniqueKey'
                  )
                )
    ...
    for (block of Array.from(this.doc.blocks)) {
      if (block instanceof ArtboardBlock && block.is_screenfull) {
        for (flexLength of ['flexWidth', 'flexHeight']) {
          s(block.componentSpec, flexLength, true)
        }
      }
    }
    ...
    if (this.props.normalizeCheckMode != null) {
      this.props.normalizeCheckMode.assert(
        () => blocks_to_normalize.length === this.doc.blocks.length
      )
    }
    ...
    for (block of Array.from(blocks_to_normalize)) {
      ...
      if (block instanceof TextBlock) {
        if (
          !this.props.skipBrowserDependentCode &&
          !config.skipBrowserDependentCode
        ) {
          const pdom = this.clean_pdom_for_geometry_computation(
            block.pdomForGeometryGetter(instanceEditorCompilerOptions)
          )

          let cache_entry = this.editorCache.blockComputedGeometryCache[
            block.uniqueKey
          ]
          if (
            cache_entry == null ||
            !static_pdom_is_equal(pdom, cache_entry.pdom)
          ) {
            ;({ height, width } = getSizeOfPdom(pdom, offscreen_node))
            this.editorCache.blockComputedGeometryCache[
              block.uniqueKey
            ] = cache_entry = { pdom, height, width }
          }

          s(block, 'computedSubpixelContentWidth', cache_entry.width)
          if (block.contentDeterminesWidth) {
            s(block, 'width', Math.ceil(block.computedSubpixelContentWidth))
          }
        ...
        willMutate = fn => {
          this.doc.leaveReadonlyMode()
          fn()
          return this.doc.enterReadonlyMode()
        }
        block.propValues.enforceValueConformsWithSpec(
        ...
        if (!source.componentSpec.flexWidth) {
          s(block, 'flexWidth', false)
        }
        if (!source.componentSpec.flexHeight) {
          s(block, 'flexHeight', false)
        }
        ...
        try {
          ;({ minWidth, minHeight } = this._getBlockMinGeometry(
            block,
            instanceEditorCompilerOptions,
            offscreen_node
          ))
          ...
          e = error
          if (config.warnOnEvalPdomErrors) {
            console.warn(e)
          }
          continue
        }
        ...
        s(
          ...
          'height',
          source.componentSpec.flexHeight
            ? Math.max(block.height, minHeight)
            : minHeight
        )
      }
    }
    ...
    for (let externalInstance of Array.from(
      _l.flatten(this.doc.blocks.map(b => b.externalComponentInstances))
    )) {
      var spec
      if (
        (spec = getExternalComponentSpecFromInstance(
          externalInstance,
          this.doc
        )) == null
      ) {
      ...
      externalInstance.propValues.enforceValueConformsWithSpec(
        spec.propControl,
        willMutate
      )
    }
    ...
    const blocksWithComponentSpecs = _l.filter(
      this.doc.blocks,
      block => block.componentSpec != null
    )
    const componentsWithSameRef = _l.pickBy(
      _l.groupBy(blocksWithComponentSpecs, 'componentSpec.componentRef'),
      arr => arr.length > 1
    )
    for (let components of Array.from(_l.values(componentsWithSameRef))) {
      var needle
      const recentlyAdded = components.filter(
        c => (
          (needle = c.uniqueKey),
          !Array.from(
            _l.map(
              this.last_normalized_doc_json != null
                ? this.last_normalized_doc_json.blocks
                : undefined,
              'uniqueKey'
            )
          ).includes(needle)
      ...
      if (recentlyAdded.length === components.length) {
        console.warn(
          'Multiple components found with the same Ref but unable to understand which was the original one'
        )
        continue
      }
    ...
    for (block of Array.from(this.doc.blocks)) {
      ...
      for (e of Array.from(Block.edgeNames)) {
        s(block.edges, e, Math.round(block.edges[e]))
      }
      ...
      if ((artboard = block.artboard) != null) {
        if (block.centerHorizontal) {
          s(
            block,
            'left',
            block.integerPositionWithCenterNear(artboard.horzCenter, 'left')
          )
        ...
        if (block.centerVertical) {
          s(
            block,
            'top',
            block.integerPositionWithCenterNear(artboard.vertCenter, 'top')
          )
    ...
    for (let stack of Array.from(this.doc.blocks)) {
      if (stack instanceof StackBlock) {
        const {
          ...
          crossStart,
          crossCenter,
        } = stack.directionHorizontal
          ? {
              start: 'left',
              length: 'width',
              crossStart: 'top',
              crossCenter: 'vertCenter',
            }
          : {
              start: 'top',
              length: 'height',
              crossStart: 'left',
              crossCenter: 'horzCenter',
            }
        const children = _l.sortBy(stack.children, start)
        const space_between =
          (stack[length] - _l.sumBy(children, length)) / (children.length + 1)
        let space_used = 0
        for (let i = 0; i < children.length; i++) {
          const stacked = children[i]
          const crossDiff = stack[crossCenter] - stacked[crossCenter]
          const mainDiff =
            stack[start] + space_used + space_between - stacked[start]
    ...
    for (let ncms of Array.from(this.doc.blocks)) {
      ...
      if (
        ncms instanceof MutlistateHoleBlock &&
        (preview_artboard = ncms.getArtboardForEditor()) != null
      ) {
    ...
    if (was_useful) {
      console.log('normalize() had an effect')
    }
    assert(() => this.doc.isInReadonlyMode())
    ...
    return (this.editorCache.render_params.mutated_blocks = _l.keyBy(
      blocks_to_normalize,
      'uniqueKey'
    ))
  },
  ...
  normalizeForceAll() {
    ...
    this.editorCache.render_params.mutated_blocks = _l.keyBy(
      this.doc.blocks,
      'uniqueKey'
    )
    this.doc.enterReadonlyMode()
  ...
  getCompiledComponentByUniqueKey(uniqueKey) {
    return memoize_on(
      ...
      () => {
        ...
        if (componentBlockTree == null) {
          return undefined
        }
        return compileComponentForInstanceEditor(
          componentBlockTree,
          this.getInstanceEditorCompileOptions()
        )
      }
  ...
  toggleMode(mode) {
    if (!this.getEditorMode().isAlreadySimilarTo(mode)) {
      ...
      return this.setEditorStateToDefault()
    }
  },
  ...
  docNameVL() {
    return {
      ...
      requestChange: value => {
        ...
        if (this.docRef != null) {
          return server.saveMetaPage(this.docRef, { url: value }, _ => null)
        }
      },
    }
  },
  ...
  selectBlocks(selection, param) {
    ...
    if (selection == null) {
      throw new Error(
        'type error, selectBlocks expected non-null `blocks` array'
      )
    }
    ...
    if (additive) {
      ...
      selection = oldSelection
        ...
        .filter(
          b =>
            !(
              Array.from(oldSelection).includes(b) &&
              Array.from(selection).includes(b)
            )
        )
    }
    ...
    if (
      union != null &&
      (artboardEnclosingAllSelectedBlocks = _l.find(this.doc.artboards, a =>
        a.contains(union)
      ))
    ) {
    ...
    return (this.selectedBlocks =
      (left = _l.uniq(selection)) != null ? left : [])
  },
  ...
  getSelectedBlocks() {
    ...
    for (let i = 0; i < this.selectedBlocks.length; i++) {
      ...
      if (
        replacement instanceof MutlistateAltsBlock &&
        (hole = _l.find(
          this.doc.blocks,
          b =>
            b instanceof MutlistateHoleBlock &&
            b.altsUniqueKey === replacement.uniqueKey
        ))
      ) {
      ...
      if (replacement != null ? replacement.locked : undefined) {
        replacement = null
      }
      if (b !== replacement) {
  ...
  setHighlightedblock(block) {
    return (this.highlightedBlock = block)
  },

  getActiveArtboard() {
    return (this.activeArtboard =
      this.activeArtboard != null ? this.activeArtboard.getBlock() : undefined)
  },

  handleSelectParent() {
    const parent = __guard__(this.getSelectedBlocks()[0], x => x.parent)
    if (parent && !parent.isDocBlock) {
      return this.selectBlocks([parent])
    }
  },
  ...
  handleSelectChild() {
    ...
    if (
      !_l.isEmpty(
        children || !Array.from(this.doc.blocks).includes(children[0])
      )
    ) {
      return this.selectBlocks([children[0]])
    }
  },
  ...
  handleSelectSibling(offset) {
    const selectedBlocks = this.getSelectedBlocks()
    if (_l.isEmpty(selectedBlocks)) {
      return
    }

    // Handles negatives
    const mod = (n, l) => ((n % l) + l) % l
    const get_next = (current, delta, lst) =>
      lst[mod(_l.indexOf(lst, current) + delta, lst.length)]

    const selected = selectedBlocks[0]
    const next_block = get_next(
      selected,
      offset,
      _l.sortBy(selected.getSiblingGroup(), ['top', 'left'])
    )
    return this.selectBlocks([next_block])
  },

  getBlockUnderMouseLocation(where) {
    return this.doc.getBlockUnderMouseLocation(where)
  },

  //# Modal UIs

  handleShortcuts() {
    const Meta = window.navigator.platform.startsWith('Mac')
      ? '⌘'
      ...
      return [
        ...
        React.createElement(
          ...
          React.createElement(
            'p',
            null,
            'Shift + Resize → Resize block w\x2F fixed ratio'
          ),
          React.createElement(
            'p',
            null,
            'Alt + Resize → Resize block from center\x2Fmiddle'
          ),
          ...
          React.createElement(
            'p',
            null,
            'Alt + Drag → Drag copy with children'
          ),
          React.createElement(
            'p',
            null,
            'Alt + Arrow Keys → Nudge block w\x2F children'
          ),
          React.createElement(
            'p',
            null,
            'Shift + Drag → Drag block perfectly vertically or horizonally'
          ),
          ...
          React.createElement(
            'p',
            null,
            's → Create blank 1024 x 1024 artboard'
          ),
          React.createElement('hr', null),
          ...
          React.createElement(
            'p',
            null,
            'Shift + ? → Open\x2Fclose shortcuts modal'
          )
        ),
        React.createElement(
          Modal.Footer,
          ...
          React.createElement(
            PdButtonOne,
            { type: 'primary', onClick: this.shortcutsModalCloseFn },
            'Close'
          )
        ),
      ]
  ...
  handleStackBlitzSave() {
    ...
    return typeof this.props.onStackBlitzShare === 'function'
      ? this.props.onStackBlitzShare()
      : undefined
  },
...
`
            ...
            : React.createElement(
                ...
                React.createElement(
                  'h5',
                  { style: { margin: '9px 0', color: 'black' } },
                  'Step 1. Install Pagedraw CLI'
                ),
                React.createElement(
                  'p',
                  null,
                  'In bash terminal (Terminal.app on macOS) run:'
                ),
                React.createElement(CodeShower, {
                  content: 'npm install -g pagedraw-cli',
                }),

                React.createElement(
                  'h5',
                  { style: { margin: '9px 0', color: 'black' } },
                  'Step 2. Login to Pagedraw '
                ),
                ...
                React.createElement(
                  'h5',
                  { style: { margin: '9px 0', color: 'black' } },
                  'Step 3. pagedraw.json'
                ),
                React.createElement(
                  'p',
                  null,
                  'In the root of your project create a file pagedraw.json with the following contents'
                ),
                React.createElement(CodeShower, {
                  content: recommended_pagedraw_json_for_app_id(
                    this.props.app_id,
                    this.doc.filepath_prefix
                  ),
                }),
                ...
                React.createElement(
                  'h5',
                  { style: { margin: '9px 0', color: 'black' } },
                  'Step 4. Pagedraw Sync\x2FPull'
                ),
                React.createElement(
                  'p',
                  null,
                  'Start Pagedraw sync process (it runs continuously):'
                ),
                ...
                React.createElement(
                  'p',
                  null,
                  'Alternatively, to one-off download the Pagedraw file changes, run'
                ),
                ...
                React.createElement(
                  ...
                  React.createElement(
                    'a',
                    { href: 'https://documentation.pagedraw.io/install/' },
                    'https:\x2F\x2Fdocumentation.pagedraw.io\x2Finstall\x2F'
                  ),
                  ' for more info.'
                )
              )
  ...
  topbarPlayButtonIsEnabled() {
    return this.getPlayStartScreen() != null
  },
  ...
  getPlayStartScreen() {
    ...
    const selected = selectedBlocks[0]
    if (selected instanceof ArtboardBlock) {
      return selected
    }
    if ((artboard = selected.getEnclosingArtboard()) != null) {
      return artboard
    }
    if (selected instanceof InstanceBlock) {
      return selected
    }
    return null
  },
  ...
  handlePlay() {
    ...
    if (!(start_screen == null)) {
      const open_url_in_new_tab_and_separare_js_context = (
        url // Open a new window with target="_blank"
      ) =>
        ...
        // to mess with.
        _l
          .extend(window.document.createElement('a'), {
            href: url,
            target: '_blank',
      ...
      return open_url_in_new_tab_and_separare_js_context(
        `/pages/${window.pd_params.page_id}/play/${start_screen.uniqueKey}/`
      )
    } else {
      return modal.show(closeHandler => [
        React.createElement(
          ...
          React.createElement(
            Modal.Title,
            null,
            'Select an Artboard to play with it'
          )
        ),
        React.createElement(
          Modal.Body,
          null,
          `\
If you have no artboards, you should start by drawing one from the `,
          React.createElement('code', null, 'Add'),
          ` menu.
See `,
          React.createElement(
            'a',
            { href: 'https://documentation.pagedraw.io/the-editor/' },
            'https:\x2F\x2Fdocumentation.pagedraw.io\x2Fthe-editor\x2F'
          ),
          ` for more details.\
`
        ),
  ...
  updateJsonFromSketch(doc_json) {
    ...
    return this.updateJsonFromDesignTool(doc_json, {
      ...
      saveLatestImport: (docRef, doc_json) =>
        server.saveLatestSketchImportForDoc(docRef, doc_json),
    })
  },
  ...
  updateJsonFromFigma(doc_json) {
    ...
    return this.updateJsonFromDesignTool(doc_json, {
      ...
      saveLatestImport: (docRef, doc_json) =>
        server.saveLatestFigmaImportForDoc(docRef, doc_json),
    })
  },
  ...
  updateJsonFromDesignTool(doc_json, { getLastImport, saveLatestImport }) {
    return getLastImport(this.docRef).then(lastImportedJson => {
      ...
      const [updated_design, base] = Array.from([
        Doc.deserialize(doc_json),
        Doc.deserialize(lastImportedJson),
      ])

      if (config.remapSymbolsToExistingComponents) {
        remapSymbolsToExistingComponents(updated_design, this.doc)
      }
  ...
  handleNewComponent() {
    ...
    if (_l.isEmpty(this.selectedBlocks)) {
      return
    }
    ...
    const largestAncestor =
      (left = _l.maxBy(
        this.doc.blocks.filter(parent => parent.isAncestorOf(root)),
        'order'
      )) != null
        ? left
        : root
    const newRootPosition = this.doc.getUnoccupiedSpace(root, {
      top: largestAncestor.top,
      left: largestAncestor.right + 100,
    })

    const [xOffset, yOffset] = Array.from([
      newRootPosition.left - root.left,
      newRootPosition.top - root.top,
    ])
    ...
    if (
      ...
      this.selectedBlocks[0] instanceof ArtboardBlock
    ) {
      artboardBlock = this.selectedBlocks[0]
      blocksToComponentize = this.doc.blockAndChildren(artboardBlock)
    } else if (
      this.selectedBlocks.length === 1 &&
      this.selectedBlocks[0] instanceof LayoutBlock
    ) {
      const originalColor = this.selectedBlocks[0].color
      artboardBlock = this.selectedBlocks[0].becomeFresh(new_members =>
        ArtboardBlockType.create(new_members)
      )
      artboardBlock.color = originalColor
      blocksToComponentize = this.doc.blockAndChildren(artboardBlock)
    } else {
      artboardBlock = new ArtboardBlock({
        top: newRootPosition.top,
        left: newRootPosition.left,
        height: root.height,
        width: root.width,
        includeColorInCompilation: false,
      })
      blocksToComponentize = _l.uniq(
        _l.flatten(this.selectedBlocks.map(b => this.doc.blockAndChildren(b)))
      )
      this.doc.addBlock(artboardBlock)
    }

    for (let block of Array.from(blocksToComponentize)) {
      block.nudge({ x: xOffset, y: yOffset })
    }

    // Create instance block at old position
    const instance = new InstanceBlock({
      sourceRef: artboardBlock.componentSpec.componentRef,
      top: oldRootGeometry.top,
      left: oldRootGeometry.left,
      width: oldRootGeometry.width,
      height: oldRootGeometry.height,
    })
  ...
  componentWillMount() {
    if (config.editorGlobalVarForDebug) {
      ...
      if (!('$b' in window)) {
        Object.defineProperty(window, '$b', {
          get() {
            return window.Editor.selectedBlocks[0]
          },
        })
      } // don't redefine it
    }
    ...
    if (this.props.page_id != null && this.props.docserver_id != null) {
      ...
      this.docRef = server.getDocRefFromId(
        this.props.page_id,
        this.props.docserver_id
      )
      ...
      return window.setTimeout(() => {
        ...
        return window.document.fonts.ready.then(() => {
          ...
          assert(() =>
            _l.isEmpty(this.other_peoples_computed_instance_min_geometries)
          )
  ...
  finishLoadingEditor(json) {
      ...
      subscribeToDevServer(function(id, errors) {
        if (errors.length > 0) {
          if (errors[0] === 'disconnected') {
            ...
            console.warn('Library code has errors. Check the CLI')
          }
          return
        }
    ...
    return Promise.all(doc.libraries.map(lib => lib.load(window)))
      .catch(function(e) {
      ...
      .then(() => {
        if (
          (this.librariesWithErrors = doc.libraries.filter(
            l => !l.didLoad(window)
          )).length > 0
        ) {
  ...
  finishLoadingDoc(doc, json) {
    ...
    if (this.doc.url == null) {
      this.doc.url = this.props.url
    }
    ...
    this.listen(window, 'resize', () =>
      this.handleDocChanged({ fast: true, subsetOfBlocksToRerender: [] })
    )
    ...
    this.viewportManager.setViewport(
      (this.is_recovering_with_recovery_state != null
        ? this.is_recovering_with_recovery_state.viewport
        : undefined) != null
        ? this.is_recovering_with_recovery_state != null
          ? this.is_recovering_with_recovery_state.viewport
          : undefined
    ...
    if (this.docRef != null) {
      server.doesLastSketchJsonExist(this.docRef).then(isSketchImport => {
    ...
    if (this.docRef != null) {
      server.kickMeWhenCommitsChange(this.docRef, () => {
        return this.handleDocChanged({
          fast: true,
          subsetOfBlocksToRerender: [],
          dontUpdateSidebars: false,
    ...
    if (window.pd_params.figma_modal) {
      this.showUpdatingFromFigmaModal()
    }

    // Send conversion event to google adwords
    return typeof window.gtag === 'function'
      ? window.gtag('event', 'conversion', {
          send_to: 'AW-821312927/yIXqCMPHn3sQn_vQhwM',
          value: 1.0,
          currency: 'USD',
        })
      : undefined
  ...
  componentWillUnmount() {
    ...
    for (let [target, event, handler] of Array.from(this._openEventListeners)) {
      target.removeEventListener(event, handler)
    }

    // Turn off the livecollab system
    return typeof this._unsubscribe_from_doc === 'function'
      ? this._unsubscribe_from_doc()
      : undefined
  },
  ...
  loadEditor2() {
    ...
    return setTimeout(() => {
      throw new Error('Editor invariant violated; bailing')
    }, (4 + 26 * Math.random()) * 1000)
  },
  ...
  handleKeyUp(e) {
    ...
    if (this.keyEventShouldBeHandledNatively(e)) {
      return
    }
    ...
    if ([8, 46].includes(e.keyCode)) {
      ...
      if (config.realExternalCode) {
        modal.show(closeHandler => [
          React.createElement(LibraryAutoSuggest, {
            focusOnMount: true,
            onChange: () => modal.forceUpdate(() => {}),
          }),
        ])
      }
      ...
      if (config.arrowKeysSelectNeighbors) {
        if (meta) {
          ...
          if (e.shiftKey) {
            keyedTowards = _l.mapValues(keyedTowards, dist => dist * 10)
          }
          if (!e.altKey) {
            blocks = _l.flatMap(blocks, b => b.andChildren())
          }
          for (block of Array.from(blocks)) {
            block.nudge(keyedTowards)
          }
        ...
        // Alt + arrow keys
        if (e.altKey) {
          for (block of Array.from(
            _.flatten(
              this.getSelectedBlocks().map(block => block.andChildren())
            )
          )) {
            block.nudge(keyedTowards)
          }
          ...
          for (block of Array.from(this.getSelectedBlocks())) {
            block.expand(keyedTowards)
          }
      ...
      this.getSelectedBlocks().forEach(block => {
        if (
          UserLevelBlockTypes.TextBlockType.describes(block) ||
          UserLevelBlockTypes.TextInputBlockType.describes(block)
        ) {
      ...
      !meta &&
      (block_to_draw = block_type_for_key_command(e.key.toUpperCase())) != null
    ) {
      ...
      if (!this.getEditorMode().keepBlockSelectionOnEscKey()) {
        this.selectBlocks([])
      }
      this.setEditorStateToDefault()
      fast = true
      ...
      let found
      if (
        (found = _l.find(this.getSelectedBlocks(), b => b instanceof TextBlock))
      ) {
      ...
      this.doc.addBlock(
        _l.extend(
          artboard,
          this.doc.getUnoccupiedSpace(artboard, { top: 100, left: 100 })
        )
      )
      ...
      if (this.shortcutsModalCloseFn == null) {
        // unless the shortcuts modal is already open
      ...
      for (let t of Array.from(tbs)) {
        t[prop] = toggle_target
      }
    } else {
      return
    }
  ...
  keyEventShouldBeHandledNatively(evt) {
    ...
    if (
      ['b', 'i', 'u', 'Escape'].includes(evt.key) &&
      Quill.dom_node_is_in_quill(d)
    ) {
      return false
    }
  ...
  _saveHistory(newState) {
    ...
    if (_l.isEqual(newState, this.undos_doc_state)) {
      return
    }
    ...
    if (config.logOnUndo) {
      return console.log(
        `Saving history. New undo stack length: ${this.undoStack.length()}`
      )
    }
  },
  ...
  handleUndo() {
    ...
    ;[this.undos_doc_state, old_doc_state] = Array.from([
      model_differ.patch(this.undos_doc_state, this.undoStack.pop()),
      this.undos_doc_state,
    ])
    ...
    if (config.logOnUndo) {
      return console.log(
        `Undid last edit. New undo stack length: ${this.undoStack.length()}`
      )
    }
  },
  ...
  handleRedo() {
    ...
    ;[this.undos_doc_state, old_doc_state] = Array.from([
      model_differ.patch(this.undos_doc_state, this.redoStack.pop()),
      this.undos_doc_state,
    ])
    ...
    if (config.logOnUndo) {
      return console.log(
        `Redid last edit. New undo stack length: ${this.undoStack.length()}`
      )
    }
  },
  ...
  initializeServerSync() {
    ...
    return (this._unsubscribe_from_doc = server.watchPage(
      this.docRef,
      (...args) => {
        let json
        ;[this.cas_token, json] = Array.from(args[0])
        if (this.props.readonly === true) {
          this.lastDocFromServer = json
        }
  ...
  generateLogCorrelationId() {
    return String(Math.random()).slice(2)
  },

  handleSave(new_doc_json) {
    // we can skip serialization if we already have a json by passing it in as an argument
    if (new_doc_json == null) {
      new_doc_json = undefined
    }
    if (new_doc_json == null) {
      new_doc_json = this.doc.serialize()
    }

    if (this.props.onChange != null) {
      this.props.onChange(new_doc_json)
    }
    ...
    if (config.flashy) {
      this.doc.intentionallyMessWithUser = true
    }
  ...
  updateJsonFromServer(json) {
    ...
    if (config.logOnSave) {
      console.log(`[${log_id}] collaborator made an edit`)
    }
    if (config.logOnSave) {
      console.log(`[${log_id}]`, json)
    }
    if (config.logOnSave && _l.isEqual(json, this.last_synced_json)) {
      console.log(`[${log_id}] should no op`)
    }
  ...
  loadForeignDocFromFullDocRef(b64_full_docref_json) {
    ...
    const docRef = foreign_server.getDocRefFromId(
      full_docref.page_id,
      full_docref.docserver_id
    )
  ...
  handleCopy(e) {
    if (this.clipboardEventTargetShouldHandle(e)) {
      return
    }
  ...
  PDClipboardData: Model.register(
    'PDClipboardData',
    (PDClipboardData = (function() {
  ...
  getUniqueDocIdentifier() {
    return window.location.origin + window.location.pathname
  },

  copySelectedBlocksToClipboard(e, callbackOnCopiedBlocks) {
    if (callbackOnCopiedBlocks == null) {
      callbackOnCopiedBlocks = function() {}
    }
    const blocks = _.uniq(
      _.flatten(this.getSelectedBlocks().map(b => b.andChildren()))
    )
    ...
    const clipboard_contents = new PDClipboardData({
      ...
      externalComponentSpecs: (() => {
        const refed_external_components = _l.uniq(
          _l.map(_l.flatMap(blocks, 'externalComponentInstances'), 'srcRef')
        )
        return this.doc.externalComponentSpecs.filter(ecs =>
          Array.from(refed_external_components).includes(ecs.ref)
        )
      })(),
    ...
    e.clipboardData.setData(
      this.PagedrawClipboardProtocolName,
      serialized_contents
    )
    ...
    return clipboardItem.getAsString(json => {
      ...
      try {
        ;({
          ...
          source_doc_id,
          externalComponentSpecs,
        } = PDClipboardData.deserialize(JSON.parse(json)))
      } catch (error) {
      ...
      for (let spec of Array.from(externalComponentSpecs)) {
        if (_l.find(this.doc.externalComponentSpecs, { ref: spec.ref })) {
          continue
        }
      ...
      const offsetToViewportCenter = box => {
        ...
        const xPosition = Math.round(
          viewport.left + viewport.width / 2 - box.width / 2
        )
        const yPosition = Math.round(
          viewport.top + viewport.height / 2 - box.height / 2
        )
        const [offset_left, offset_top] = Array.from(
          [xPosition - box.left, yPosition - box.top].map(Math.round)
        ) // round so we end up on integer top/left values
      ...
      if (source_doc_id !== this.getUniqueDocIdentifier()) {
        ;[offset_left, offset_top] = Array.from(
          offsetToViewportCenter(bounding_box)
        )
      } else if (
        activeArtboard != null &&
        bounding_box != null &&
        (sourceArtboard = _l.find(this.doc.artboards, a =>
          a.contains(bounding_box)
        ))
      ) {
        ...
        ;[offset_left, offset_top] = Array.from([
          activeArtboard.left - sourceArtboard.left,
      ...
      const final_destination = new Block({
        top: bounding_box.top + offset_top,
        left: bounding_box.left + offset_left,
        width: bounding_box.width,
        height: bounding_box.height,
      })
      if (
        !final_destination.overlaps(
          new Block(this.viewportManager.getViewport())
        )
      ) {
      ...
      const [minX, minY] = Array.from(
        ['left', 'top'].map(prop => _l.minBy(blocks, b => b[prop])[prop])
      )
      if (minX < 0) {
        for (block of Array.from(blocks)) {
          block.left -= minX
  ...
  pasteSvg(plain_text, parsed_svg) {
    ...
    const xPosition = Math.round(
      viewport.left + (viewport.width / 2 - width / 2)
    )
    const yPosition = Math.round(
      viewport.top + (viewport.height / 2 - height / 2)
    )

    const image_block = new ImageBlock({
      top: yPosition,
      left: xPosition,
      width,
      height,
  ...
  pastePng(clipboardItem) {
    ...
    if (blob == null) {
      return track_warning('PNG image could not be extracted as a file', {
        clipboardItem,
      })
    }
  ...
  pastePlainText(clipboardItem) {
    return clipboardItem.getAsString(plain => {
      ...
      const block = this.doc.addBlock(
        new TextBlock({
          htmlContent: Dynamicable(String).from(plain),
          top: 100,
          left: 100,
          width: 100,
          height: 17,
      ...
      if (item.type === this.PagedrawClipboardProtocolName) {
        ...
        item.getAsString(plain => {
          ...
          if ((parsed_svg = util.parseSvg(plain)) != null) {
            ...
            return console.warn(
              `Trying to paste unrecognized plain text: ${plain}`
            )
          }
        })
      } else {
  ...
  clipboardEventTargetShouldHandle(evt) {
    if (this.keyEventShouldBeHandledNatively(evt)) {
      return true
    }
  ...
  selectAndMoveToBlocks(blocks) {
    ...
    this.viewportManager.centerOn(
      blocks.length === 1 ? blocks[0] : Block.unionBlock(blocks)
    )
  ...
  cache_error_and_warning_messages() {
    ...
    return (
      ([this.errors, this.warnings] = Array.from(
        (ref = this.error_and_warning_messages_for_doc())
      )),
      ref
    )
  },
  ...
  error_and_warning_messages_for_doc() {
    ...
    const error = (content, handleClick) =>
      error_messages.push({ content, handleClick })
    const warning_messages = []
    const warning = (content, handleClick) =>
      warning_messages.push({ content, handleClick })

    const { doc } = this
    doc.inReadonlyMode(() => {
      const components = doc.getComponents()

      _l.toPairs(_l.groupBy(components, filePathOfComponent)).forEach(
        (...args) => {
      ...
      var artboardsInsideArtboards = function({ block, children }, inArtboard) {
        ...
        const childrenResults = _l.flatten(
          children.map(child =>
            artboardsInsideArtboards(
              child,
              block instanceof ArtboardBlock || inArtboard
            )
          )
        )
        if (inArtboard && block instanceof ArtboardBlock) {
          return [block].concat(childrenResults)
        } else {
      ...
      artboardsInsideArtboards(doc.getBlockTree()).forEach(artboard => {
        return error(
          `Artboard ${artboard.getLabel()} inside other artboard`,
          () => {
      ...
      var nonPagesInScreenSizeBlocks = function({ block, children }, inSSB) {
        ...
        const childrenResults = _l.flatten(
          children.map(child =>
            nonPagesInScreenSizeBlocks(
              child,
              block instanceof ScreenSizeBlock || inSSB
            )
          )
        )
        if (inSSB && block instanceof ArtboardBlock && !block.is_screenfull) {
          return [block].concat(childrenResults)
        } else {
          return childrenResults
        }
      ...
      nonPagesInScreenSizeBlocks(doc.getBlockTree()).forEach(artboard => {
        return error(
          `Artboard ${artboard.getLabel()} inside Screen Size Group, but is not page`,
          () => {
            this.setSidebarMode('draw')
            return this.selectAndMoveToBlocks([artboard])
          }
      ...
      doc.blocks
        .filter(
          b => b instanceof InstanceBlock && b.getSourceComponent() == null
        )
        .forEach(instance => {
          return error(
            `Instance block ${instance.getLabel()} without a source`,
            () => {
      ...
      const blocksByLocalUserFonts = _l.groupBy(
        doc.blocks.filter(b => b.fontFamily instanceof LocalUserFont),
        'fontFamily.name'
      )
      ...
      var flexInsideNonFlexWidth = ({ block, children }) => {
        ...
        if (
          ...
          _l.some(
            children,
            c =>
              (c.block != null ? c.block.flexWidth : undefined) ||
              (c.block != null ? c.block.flexMarginLeft : undefined) ||
              (c.block != null ? c.block.flexMarginRight : undefined)
          )
        ) {
          return [block].concat(childrenResults)
        } else {
      ...
      flexInsideNonFlexWidth(this.doc.getBlockTree()).forEach(parent => {
        ...
        return warning(
          `Parent block ${parent.getLabel()} is not ${text} but it has horizontally flexible children`,
          () => {
            this.setSidebarMode('draw')
      ...
      var flexInsideNonFlexHeight = ({ block, children }) => {
        const childrenResults = _l.flatten(
          children.map(flexInsideNonFlexHeight)
        )
        const isFlex = is_componentable(block)
          ? block.getRootComponent().componentSpec.flexHeight
          : block != null
          ? block.flexHeight
          : undefined
        if (
          ...
          _l.some(
            children,
            c =>
              (c.block != null ? c.block.flexHeight : undefined) ||
              (c.block != null ? c.block.flexMarginTop : undefined) ||
              (c.block != null ? c.block.flexMarginBottom : undefined)
          )
        ) {
          return [block].concat(childrenResults)
        } else {
      ...
      return flexInsideNonFlexHeight(this.doc.getBlockTree()).forEach(
        parent => {
          const text = is_componentable(parent)
            ? 'resizable height'
            : 'flex height'
          return warning(
            `Parent block ${parent.getLabel()} is not ${text} but it has vertically flexible children`,
            () => {
              this.setSidebarMode('draw')
              return this.selectAndMoveToBlocks([parent])
            }
