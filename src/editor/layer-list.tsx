...
export default createReactClass({
    ...
    isCollapsed(block) {
        // defaults to closed
        return this.collapsedBlocksById[block.uniqueKey] != null ? this.collapsedBlocksById[block.uniqueKey] : true;
    },
    ...
    componentWillReceiveProps(nextProps) {
        const selectionChanged = !isPermutation(nextProps.selectedBlocks, this.props.selectedBlocks.map(b => b.getBlock()));
        if (selectionChanged) { return this.openFoldersForBlocks(nextProps.doc, nextProps.selectedBlocks); }
    },
    ...
    getTreeList() {
        ...
        var appendToTreeList = (blockNode, depth) => {
            ...
            if (!this.isCollapsed(blockNode.block)) { return Array.from(blockNode.children).map((child) => appendToTreeList(child, depth+1)); }
        };

        for (let root of Array.from(block_tree_root.children)) { appendToTreeList(root, 0); }
    ...
    render() {
        ...
        const treeListView = this.getTreeList().map(({block, depth, hasChildren}) => {
        ...
        return React.createElement("div", {"className": "layer-list editor-scrollbar scrollbar-show-on-hover bootstrap"},
            (!_l.isEmpty(this.props.doc.blocks) ? React.createElement("div", {"onClick": (() => { this.props.onBlocksSelected([], {additive: false}); return this.props.onChange({fast: true}); }),  
                "style": ({paddingLeft: 15, display: 'flex', justifyContent: 'space-between'}),  
                "className": `layer-list-item ${_l.isEmpty(this.props.selectedBlocks) ? 'selected' : ''}`},
                React.createElement("div", null, "Doc"),
                React.createElement("div", {"style": ({maxWidth: 100, whiteSpace: 'nowrap', overflowX: 'hidden', textOverflow: 'ellipsis'})}, (this.props.doc.url))
            ) : undefined),
          ...
          if (!_l.isEqual(selectedBlocks, prevSelectedBlocks)) {
              return (() => {
                  ...
                  for (let blockKey of Array.from(selectedBlocks)) {
                      ...
                      if (item != null) { result.push(__guardMethod__(ReactDOM.findDOMNode(item), 'scrollIntoViewIfNeeded', o => o.scrollIntoViewIfNeeded())); } else {
    ...
    handleLayerItemMouseDown(e, block) {
      ...
      if (e.buttons === 1) { this.props.onBlocksSelected([block], {additive: (e.metaKey || e.ctrlKey || e.shiftKey)}); }
    ...
    rerenderHighlight(blocks) {
      ...
      if (this.blocks_to_rerender_highlight == null) { this.blocks_to_rerender_highlight = {}; }

      // add blocks to the set of blocks we need to rerender
      for (let block of Array.from(blocks)) { if (block != null) { this.blocks_to_rerender_highlight[block.uniqueKey] = true; } }

      if (needs_frame) { return window.requestAnimationFrame(() => {
        this.props.onChange({fast: true, dontUpdateSidebars: false, subsetOfBlocksToRerender: _l.keys(this.blocks_to_rerender_highlight)});
...
var LayerListItem = createReactClass({
  render() {
    const {labelValueLink, depth, hasChildren, isSelected, isCollapsed, isBeingRenamed, isLockedValueLink, block, parentLayerList} = this.props;
    return React.createElement("div", {"className": (`layer-list-item ${isSelected ? 'selected' : ''} ${depth === 0 ? 'top-level' : ''}`),  
      "style": ({
        paddingLeft: 25 + (15 * depth)
      }),  
      "onMouseDown"(e) { return parentLayerList.handleLayerItemMouseDown(e, block); },  
      "onMouseOver"() { return parentLayerList.handleMouseOver(block); },  
      "onMouseLeave"() { return parentLayerList.handleMouseLeave(block); }},
        React.createElement("div", {"className": "layer-list-item-line"},
          ( hasChildren ?
             ...
             React.createElement("div", {"onMouseDown"(e) { return parentLayerList.handleToggleCollapsed(e, block); }, "className": "layer-list-collapser"},
                  React.createElement("div", {"className": "layer-list-collapser-triangle", "style": ({
                      borderColor: `transparent transparent transparent ${isSelected ? '#fff' : '#8c8c8c'}`,
          ...
          React.createElement(EditableText, { 
             ...
             "onSwitchToEditMode"(isEditMode) { return parentLayerList.handleEditableTextSwitchToEditMode(block, isEditMode); },  
             "editingStyle": ({width: '100%'})}),

          ( !isBeingRenamed ?
              React.createElement("div", {"className": (block.locked ? "locked" : undefined), "style": ({flexShrink: 0, marginLeft: 5})},
  ...
  shouldComponentUpdate(nextProps) {
    return !(
      ...
      ((nextProps.isBeingRenamed === this.props.isBeingRenamed && this.props.isBeingRenamed === false) && 
      ...
      (nextProps.isCollapsed === this.props.isCollapsed) && nextProps.isLockedValueLink.value === this.props.isLockedValueLink.value)
    );
}
});
